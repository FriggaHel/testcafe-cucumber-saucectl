...
exports.createSauceJson = async (reportsFolder, xunitReport) => {
  ...
  for (let testsuite of result.testsuites.testsuite) {
    nativeLog.total_tests += parseInt(testsuite.tests);
    nativeLog.total_success += parseInt(testsuite.tests) - parseInt(testsuite.errors);
    nativeLog.total_failures += parseInt(testsuite.errors);
    nativeLog.total_time += parseInt(testsuite.time);
  ...
  fs.writeFileSync(nativeLogFile, JSON.stringify(nativeLog, '', 2));
  fs.writeFileSync(jsonLogFile, JSON.stringify(jsonLog, '', 2));
